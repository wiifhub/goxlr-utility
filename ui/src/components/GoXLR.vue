<template>
  <div id="main" class="broadcast-console">
    <DeviceSelector v-if="!isDeviceSet()"/>
    <template v-if="isDeviceSet()">
      <header class="console-header">
        <div class="console-brand" aria-label="GoXLR Utility Plus">
          <span class="console-brand-mark">GX</span>
          <span>
            <strong>GoXLR</strong>
            <small>UTILITY PLUS</small>
          </span>
        </div>
        <div class="console-status" role="status" aria-live="polite">
          ...
          <span>{{ getActiveProfileName() }}</span>
        </div>
      </header>

      <h1 class="screenreader-only"> {{ $t('message.navigation.accessibilityProfileSection') }}</h1>
      <div class="console-overview">
        <section class="console-panel profile-panel" :aria-label="$t('message.navigation.accessibilityProfileSection')">
          <div class="panel-label">LIBRARY</div>
          <FileTabs/>
        </section>
        <section class="console-panel device-panel" aria-label="GoXLR device preview">
          <div class="panel-label">HARDWARE</div>
          <div aria-hidden="true" class="device-visualiser">
      ...
      <Tabs variant="workspace" ref="device-tabs" @on-change="onTabChange" :label="$t('message.navigation.accessibilityDeviceSection')">
...
export default {
  ...
  methods: {
    ...
    getActiveProfileName() {
      ...
      return device && device.profile_name ? device.profile_name.toUpperCase() : "NO PROFILE";
    },
